...
"""
primaryTableWhereStatement = "exists (select 1 from encounter e1 where {alias}patient_id = e1.patient_id and exists (select 1 from diagnosis d1 where d1.encounter_id = e1.encounter_id and ((d1.icd_code like 'N17%' and d1.icd_type = 10) or (d1.icd_code like '584%' and d1.icd_type = 9))))"
setupAddPrimaryTable("patient", keyColumnName = "patient_id", whereMarkers = [("ckd", primaryTableWhereStatement)])
setupAddSecondaryTable("encounter", columnNames = ["patient_id, encounter_date"], keyColumnName = "patient_id", parentTableName = "patient", parentKeyColumnName = "patient_id", orderBy = [("encounter_date", False)])
